...
simulation mismodelling. Then a series of tests of the background estimate is performed 
in control regions to establish the correctness of the method and to asses
...
of the event counts in other regions:
\begin{itemize}
\item FG/B, EG/C, EF/D - these combinations use one selection always applied, while applying and inverting the 
two other selections;
\item DG/A, BE/A, CF/A - these choices combine two selections together and apply or invert it with the 
remaining selection.
\end{itemize}

A seventh background prediction, which minimizes statistical uncertainty, BCD/A$^2$, can be constructed from regions
with at least 2 selections inverted where background event counts are largest. 
...
of events and all of the 7 background estimates which were described
...
The compatibility between predicted and observed background is estimated with a {\it p-value} for each measurement. 
...
We select the data using exactly one out of every ten luminosity sections, where a luminosity
section is a period of approximately 23 seconds of active data taking. This way of choosing 
the data is sensitive to possible issues that occur only for selected data taking periods, and also to effects that may arise from
